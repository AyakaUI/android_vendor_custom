...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.ayakaui.device | grep -q "$AYAKA_BUILD"); then
...
function ayakauiremote()
{
    ...
    git remote rm ayakaui 2> /dev/null
    ...
    local AYAKAUI_USER=$(git config --get review.review.ayakaui.net.username)
    if [ -z "$AYAKAUI_USER" ]
    then
        git remote add ayakaui ssh://review.ayakaui.net:29418/$PFX$PROJECT
    else
        git remote add ayakaui ssh://$AYAKAUI_USER@review.ayakaui.net:29418/$PFX$PROJECT
    fi
    echo "Remote 'ayakaui' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.ayakaui.device | grep -q "$AYAKA_BUILD");
    then
...
function ayakauigerrit() {
    ...
    local user=`git config --get review.review.ayakaui.net.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "ayakauigerrit" ]; then
...
function ayakauirebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      ayakauirebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.ayakaui.net/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.ayakaui.device | grep -q "$AYAKA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
